# 31. zed-lock: evented cross-platform process locking

**Status:** architecture approved; the `zed-cli` prototype and focused
conformance coverage landed in
[`zed-pkg/zed-cli#178`](https://github.com/zed-pkg/zed-cli/pull/178); standalone
`zed-pkg/zed-lock` extraction is tracked by
[GitHub issue #180](https://github.com/zed-pkg/zed-cli/issues/180) and
...
- macOS: the independently certified native whole-file advisory-lock backend;
...
- Lock paths live in a private Zed lock directory and are keyed by a canonical
  lock class plus artifact, build, project, or metadata identity.
...
Timeout is caller policy, not a reason to restart native acquisition in a
...
- [canonical Linear architecture document](https://linear.app/denman/document/zed-lock-event-driven-cross-platform-locking-architecture-and-b19dc7a81fe5)
